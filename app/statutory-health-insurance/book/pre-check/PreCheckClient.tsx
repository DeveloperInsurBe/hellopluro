...
const cardBaseClass =
  "bg-white/92 backdrop-blur-sm border border-[#d6e5ff] rounded-2xl shadow-[0_20px_45px_rgba(18,63,130,0.08)]";
...
export default function PreCheckClient({
  ...
  return (
    <div className="bg-[radial-gradient(circle_at_20%_0%,#eaf5ff_0,#f7fbff_38%,#f4f8ff_100%)] min-h-screen py-6 md:py-10">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        <AnimatePresence mode="wait">
        {phase === "precheck" && (
          <motion.section key="phase-precheck" {...fadeInUp} className={`${cardBaseClass} p-5 md:p-7`}>
            <h1 className="text-2xl md:text-3xl font-semibold text-[#0b2c6b]">Pre-check</h1>
            <p className="mt-4 rounded-xl bg-[#dcf7ff] text-[#0673a8] p-3.5 text-sm md:text-base">
              Please note that you will need the following documents to book your insurance.
            </p>

            <h2 className="mt-6 text-xl md:text-2xl font-semibold text-[#0b2c6b]">
              Your documents (required)
            </h2>
            <p className="mt-2 text-[#2d4d7f] text-sm md:text-base">
              Please confirm each required document is ready before you continue.
            </p>

            <div className="mt-5 space-y-3">
              <label className="flex items-center gap-3 text-base text-[#0e3570] font-medium">
                <input
                  type="checkbox"
                  checked={passportReady}
                  onChange={(e) => setPassportReady(e.target.checked)}
                  className="h-5 w-5 rounded border-[#9ab8e8]"
                />
        ...
        {phase === "customerType" && (
          <motion.section key="phase-customerType" {...fadeInUp} className={`${cardBaseClass} p-5 md:p-7`}>
            <h1 className="text-2xl md:text-3xl font-semibold text-[#0b2c6b]">
              Get Started with your Statutory Health Insurance application
            </h1>
            <p className="mt-3 text-[#2d4d7f] text-sm md:text-base">
          ...
          <motion.section key="phase-wizard" {...fadeInUp} className={`${cardBaseClass} p-4 md:p-7`}>
            ...
            {wizardStep === 2 && (
              <StepCard title="Let's Get Started with your insurance application" subtitle="Fill in the form below to obtain your German Health Insurance for your visa application.">
                ...
                <label className="mt-6 flex items-start gap-3 text-[#244a7d]">
                  <input
                    type="checkbox"
            ...
            {wizardStep === 4 && (
              <StepCard title="Continue with providing your address" subtitle="Fill the form below to obtain your German Health Insurance for Visa.">
                ...
                <label className="mt-4 flex items-center gap-3 text-[#244a7d]">
                  <input
                    type="checkbox"
                ...
                <div className="mt-3 space-y-4">
                  ...
                  />
                  <div>
                    <p className="mb-2 text-[#0e3570] font-semibold">
            ...
            {wizardStep === 7 && (
              <StepCard title="Review and Declaration" subtitle="Please review your statutory health insurance details carefully before submitting.">
                ...
                <div className="mt-8 space-y-3">
                  <label className="flex items-start gap-3 text-[#244a7d]">
                    <input
                      type="checkbox"
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 md:gap-4">
          <button
            ...
            className="px-6 py-2.5 rounded-xl border border-[#c8dbff] bg-white text-[#0b2c6b] font-semibold cursor-pointer transition hover:shadow-[0_6px_16px_rgba(11,44,107,0.12)] hover:-translate-y-0.5 disabled:opacity-45 disabled:cursor-not-allowed"
          >
            Back
          </button>
          ...
          {phase === "wizard" && wizardStep === 7 ? (
            <button
              ...
              className="min-w-[220px] px-6 py-2.5 rounded-xl bg-[linear-gradient(135deg,#0000FF,#0036d1)] text-white font-semibold cursor-pointer transition hover:shadow-[0_10px_20px_rgba(0,0,255,0.28)] hover:-translate-y-0.5 disabled:bg-[#cfd8e6] disabled:shadow-none disabled:translate-y-0 disabled:cursor-not-allowed"
            >
              Book now
            </button>
          ) : (
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-semibold ${
                    isActive
                      ? "bg-[#08b1dc] text-white"
                      : isDone
                      ? "bg-[#0b2c6b] text-white"
                      : "bg-white border border-[#9ab8e8] text-[#0b2c6b]"
                  }`}
                >
                  {stepNumber}
                </div>
                <span className="text-[#1f3f74] whitespace-nowrap text-sm">{label}</span>
              </div>
              {stepNumber < stepItems.length && (
                <div className="mx-2 h-px flex-1 bg-[#c9ddff]" />
              )}
            </div>
          );
        })}
...
function StepCard({
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-[#dbe9ff] bg-gradient-to-b from-white to-[#fbfdff] p-4 md:p-6 shadow-[0_10px_26px_rgba(15,76,145,0.08)]"
    >
      <h2 className="text-xl md:text-2xl font-semibold text-[#0b2c6b]">{title}</h2>
      <p className="mt-1.5 text-[#456290] text-sm md:text-base">{subtitle}</p>
...
function SectionTitle({
  ...
  return <h3 className={`text-lg md:text-xl font-semibold text-[#0b2c6b] ${className}`}>{children}</h3>;
}
  ...
  return (
    <button
      ...
      className={`w-full text-left rounded-2xl border p-4 transition-all duration-200 cursor-pointer ${
        selected
          ? "border-primary bg-[#eef4ff] shadow-[0_8px_18px_rgba(0,0,255,0.14)]"
          : "border-[#d9e8ff] bg-white hover:border-[#9ec4ff] hover:-translate-y-0.5 hover:shadow-[0_10px_18px_rgba(18,63,130,0.12)]"
      }`}
    >
      <p className="text-lg md:text-xl font-semibold text-[#0b2c6b]">{title}</p>
      <p className="text-[#355687] mt-1 text-sm md:text-base">{description}</p>
    </button>
  );
}
  ...
  return (
    <button
      ...
      className={`text-left rounded-2xl border p-4 transition-all duration-200 cursor-pointer ${
        selected
          ? "border-[#31bfe5] bg-[#e7faff] shadow-[0_10px_22px_rgba(13,149,215,0.18)]"
          : "border-[#d9e8ff] bg-white hover:border-[#9ec4ff] hover:-translate-y-0.5 hover:shadow-[0_10px_20px_rgba(18,63,130,0.10)]"
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        <p className="text-2xl font-semibold text-[#0b2c6b]">{name}</p>
        {recommended && (
          <span className="rounded-full bg-[#20d4a7] px-3 py-1 text-sm text-[#07445a] font-semibold">
            Recommended
          </span>
        )}
      </div>
      <ul className="mt-3 space-y-1 text-sm">
        {points.map((point) => (
          <li key={point} className="flex items-center gap-2 text-[#2b5688]">
            <Check className="w-4 h-4 text-[#00a7d1]" />
            <span>{point}</span>
          </li>
        ))}
      </ul>
...
function InputField({
  ...
  return (
    <label className="block">
      <span className="block mb-1 text-[#0e3570] text-sm font-semibold">{label}</span>
      <input
        type={type}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full h-11 rounded-xl border border-[#c8dbff] bg-[#f8fbff] px-3.5 text-sm text-[#143a73] outline-none transition focus:ring-2 focus:ring-[#a6c6ff]"
      />
    </label>
  );
}
...
function PhoneField({
  ...
  return (
    <div>
      <p className="mb-1 text-[#0e3570] text-sm font-semibold">Phone Number *</p>
      <div className="flex gap-2">
        <div className="w-[130px] h-11 rounded-xl border border-[#c8dbff] bg-[#f8fbff] px-3 flex items-center gap-2">
          <span className="text-lg">{flag}</span>
          <input
            type="text"
            value={dialCode}
            onChange={(e) => onDialCodeChange(normalizeDialCode(e.target.value))}
            className="w-full bg-transparent text-sm text-[#143a73] outline-none"
            placeholder="+1"
          />
        </div>
        <input
          ...
          className="flex-1 h-11 rounded-xl border border-[#c8dbff] bg-[#f8fbff] px-3.5 text-sm text-[#143a73] outline-none focus:ring-2 focus:ring-[#a6c6ff]"
...
function FileField({
  ...
  return (
    <div>
      <p className="mb-2 text-[#0e3570] text-sm font-semibold">{label}</p>
      <label className="h-36 rounded-2xl border-2 border-dashed border-[#78b9ff] bg-[#f6fbff] flex flex-col items-center justify-center gap-2 cursor-pointer transition hover:bg-[#eef7ff]">
        <Upload className="w-6 h-6 text-primary" />
        <span className="text-[#0b2c6b] text-sm font-semibold">
          {file ? "Click to replace file" : "Click to upload"}
        </span>
        <span className="text-[#5a76a8] text-sm">PNG, PDF, JPG (max 5MB)</span>
        <input
          type="file"
      ...
      <p className="mt-2 text-sm text-[#3e649a] min-h-5">{file?.name ?? ""}</p>
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`h-11 rounded-xl border text-left px-4 text-sm font-semibold cursor-pointer transition-all duration-200 ${
        selected
          ? "border-primary bg-[#eef4ff] text-[#0b2c6b]"
          : "border-[#c8dbff] bg-white text-[#335888] hover:border-[#9ec4ff]"
      }`}
    >
      {label}
    </button>
...
function ReviewGrid({ data }: { data: ApplicationData }) {
  return (
    <div className="rounded-xl border border-[#d6e5ff] bg-[#fbfdff] p-4 md:p-6">
      <div className="grid gap-3 md:grid-cols-2 text-[#1f3f74] text-sm">
...
function ReviewItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-sm text-[#6a86b2]">{label}</p>
